...
class GraphModel:
    ...
    def __init__(
        ...
        self.__context_tensor = torch.tensor(
            [self.__path_to_context(path) for path in paths], device=self.device
        )
        self.__graph_embeddings = torch.tensor(
            self.graph_embeddings, device=self.device
        )
    ...
    def build_vocab(paths: list[str]) -> tuple[list[str], dict[str, int]]:
        ...
        print(
            f"  Found {len(tokens)} tokens and {len(distinct_paths)} distinct paths in {len(paths)} entries"
        )
    ...
    def __init_model(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(
                self.n_embedding * self.context_length,
                self.n_embedding * self.context_length,
            ),
            nn.ReLU(),
            nn.Linear(self.n_embedding * self.context_length, self.n_hidden),
            nn.ReLU(),
    ...
    def __get_graph_embeddings(self) -> np.ndarray:
        graph2vec_path = (
            self.base_model_path / ".." / ".." / "graph2vec.pkl"
        )  # TODO: revert
    ...
    def __train_model(self, epochs: int):
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % log_interval == 0:
                ...
                print(
                    f"  ({epoch+1:{num_digits}d}/{epochs:{num_digits}d}) log(loss) = {np.average(loss_samples):.6f}"
                )
                loss_samples = []
    ...
    def predict(self, paths: list[str]) -> list[Tree]:
        ...
        embedding_differences = self.__graph_embeddings.unsqueeze(
            0
